        ...
        <form action="{{ $action }}" method="POST" id="outboundForm">
            ...
            <div class="border border-gray-200 rounded-xl overflow-hidden shadow-sm bg-white">
                ...
                <div id="warehouse_warning" class="m-4 text-orange-600 bg-orange-50 p-3 rounded-lg text-sm font-medium flex items-center border border-orange-100">
                    ...
                    Vui lòng chọn Nhà Kho ở phía trên để load dữ liệu tồn kho tổng.
                </div>

                <div class="overflow-x-auto">
                    <!-- ĐÃ FIX: Xóa thẻ tbody bọc bên ngoài sai cấu trúc -->
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        warehouseSelect.addEventListener('change', async function() {
            ...
            // Xóa hết các tbody cũ (trừ thead)
            ...
            try {
                ...
                btnAddBlock.innerHTML = '+ Thêm Sản Phẩm';
        ...
        function generateProductOptions(selectedPid = null) {
            ...
            for (let pid in uniqueProducts) {
                ...
                html += `<option value="${p.id}" ${sel}>${p.name} (Tồn: ${p.total})</option>`;
            }
        ...
        // THÊM 1 KHỐI SẢN PHẨM MỚI (Mỗi khối là 1 thẻ TBODY độc lập)
        function addBlock(prePid = null, preQty = null, preDetails = null) {
            let tbody = document.createElement('tbody');
            tbody.className = "border-b-[12px] border-white shadow-sm"; // Tạo khoảng cách giữa các khối sản phẩm
            
            let maxTotal = prePid && uniqueProducts[prePid] ? uniqueProducts[prePid].total : 0;
            let displayMax = prePid ? maxTotal : '0';

            tbody.innerHTML = `
                <!-- Dòng Master: Chọn Sản Phẩm -->
            ...
            mainTable.appendChild(tbody); // Đã Fix: Nối trực tiếp tbody vào table
            ...
            prodSelect.addEventListener('change', function() {
                ...
                qtyInput.value = '';
                // Xóa hết các dòng chi tiết cũ
                tbody.querySelectorAll('.detail-row').forEach(row => row.remove());
                qtyInput.focus();
            });

            // EVENT: Gõ số lượng -> TỰ ĐỘNG ĐỀ XUẤT (AUTO-ALLOCATE)
            qtyInput.addEventListener('input', function() {
                ...
                // Chặn gõ lố tổng tồn kho
                let maxT = parseInt(this.max);
                if (maxT && reqQty > maxT) {
                    alert('Số lượng yêu cầu vượt quá tổng tồn kho đang có!');
                    this.value = maxT;
                    reqQty = maxT;
                }
        ...
        function autoAllocate(tbody, pid, reqQty) {
            ...
            if (remaining > 0) {
                ...
                trAlert.innerHTML = `<td colspan="4" class="px-5 py-2 text-red-600 font-bold text-sm text-center">⚠️ Kho bị thiếu ${remaining} SP để đáp ứng đủ yêu cầu này!</td>`;
                tbody.appendChild(trAlert);
            }
        }

        // RENDER 1 DÒNG ĐỀ XUẤT (Vị trí & Lô) - ĐÃ FIX SỬ DỤNG TR BẰNG VỚI CỘT BẢNG CHÍNH
        function renderDetailRow(tbody, pid, locId, batchId, qty) {
            ...
            tr.innerHTML = `
                <!-- Ô này dùng colspan=2 để chứa khoảng trắng mồi (như mũi tên rẽ nhánh) và Select box -->
        ...
        // Cập nhật lại input ẩn khi NV Kho tự tay đổi Kệ trong dropdown đề xuất
