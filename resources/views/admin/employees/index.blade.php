    ...
    <div class="bg-white rounded-lg shadow-sm">
        <div class="p-4 border-b border-gray-100 flex justify-between items-center">
            <form action="" method="GET" class="flex gap-2">
                ...
                <button type="submit" class="bg-gray-100 px-3 py-1.5 rounded text-sm hover:bg-gray-200">
                    Tìm
                </button>
            </form>
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4">
                            @foreach ($emp->roles as $role)
                                <span
                                    class="bg-gray-100 text-gray-700 px-2 py-0.5 rounded text-xs">{{ $role->name }}</span>
                            @endforeach
                        </td>
                        <td class="px-6 py-4">
                            @if ($emp->user->is_active)
                                <span
                                    class="text-green-600 bg-green-50 px-2 py-1 rounded-full text-xs font-bold">Active</span>
                            @else
                                <span
                                    class="text-red-600 bg-red-50 px-2 py-1 rounded-full text-xs font-bold">Inactive</span>
                            @endif
                        </td>
                        <td class="px-6 py-4 text-right space-x-2">
                            @can('update', $emp)
                                <a href="{{ route('employees.edit', $emp->id) }}" class="text-blue-600 hover:text-blue-800"><i
