...
<body class="bg-slate-50 max-h-screen w-screen flex items-center justify-center p-4 lg:p-0">
    ...
    <div class="w-screen h-screen bg-white overflow-hidden flex flex-col lg:flex-row border border-slate-100 relative">
        ...
        <div
            class="hidden  lg:w-6/12 lg:flex relative bg-slate-900 overflow-hidden flex-col justify-between p-12 text-white">

            <div class="absolute inset-0 z-0">
                <img src="https://images.unsplash.com/photo-1586528116311-ad8dd3c8310d?ixlib=rb-4.0.3&auto=format&fit=crop&w=1600&q=80"
                    alt="Warehouse" class="w-full h-full object-cover opacity-40 mix-blend-overlay">
            ...
            <div class="relative z-10 mb-10">
                ...
                <div class="grid grid-cols-2 gap-6 mt-8">
                    <div
                        class="bg-white/10 backdrop-blur-sm p-4 rounded-xl border border-white/10 hover:bg-white/20 transition duration-300">
            ...
            <div
                class="relative z-10 flex justify-between items-end text-xs text-slate-400 border-t border-white/10 pt-6">
        ...
        <div class="w-full lg:w-6/12 bg-white p-8 md:p-16 flex flex-col justify-center relative">
            ...
            <div class="max-w-md w-full mx-auto">
                <div class="mb-10">
                    <h1 class="text-3xl font-bold text-slate-900 mb-2">Chào mừng trở lại</h1>
                    <p class="text-slate-500 mb-8">Vui lòng nhập thông tin để truy cập hệ thống.</p>
                </div>

                <div class="flex p-1 bg-slate-100 rounded-xl mb-8">
                    <button onclick="switchRole('client')" id="btn-client"
                        class="flex-1 py-2.5 text-sm font-bold rounded-lg shadow-sm bg-white text-slate-900 transition-all">Khách
                        hàng</button>
                    <button onclick="switchRole('staff')" id="btn-staff"
                        class="flex-1 py-2.5 text-sm font-bold rounded-lg text-slate-500 hover:text-slate-700 transition-all">Nhân
                        viên</button>
                </div>
                ...
                <form action="{{ route('login') }}" method="POST" class="space-y-5">
                    ...
                    @if ($errors->any())
                        <div
                            class="bg-red-50 text-red-600 p-4 rounded-xl text-sm border border-red-100 flex items-start gap-3">
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2 ml-1">Email công việc</label>
                        <div class="relative group">
                            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                <i
                                    class="fa-regular fa-envelope text-slate-400 group-focus-within:text-blue-600 transition-colors"></i>
                            </div>
                            <input type="text" name="username" value="{{ old('username') }}"
                                ...
                                placeholder="VD: admin@company.com" required autofocus>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative group">
                            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                <i class="fa-solid fa-lock-open text-slate-400 group-focus-within:text-blue-600"></i>
                            </div>

                            <input type="password" name="password"
                                class="w-full pl-11 pr-4 py-3.5 bg-slate-50 border border-slate-200 rounded-xl text-sm"
                    ...
                    <button type="submit"
                        class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 rounded-lg shadow-sm shadow-blue-600/30 transition-all duration-300 transform active:scale-[0.98] flex justify-center items-center gap-2">
                        <span>Đăng nhập</span>
                ...
                <div class="relative flex py-8 items-center">
                    ...
                    <span class="shrink-0 mx-4 text-slate-400 text-xs font-medium uppercase tracking-wider">Hoặc đăng
                        nhập với</span>
                    <div class="grow border-t border-slate-200"></div>
                </div>
    ...
    <script>
        function switchRole(role) {
            ...
            if (role === 'client') {
                btnClient.className =
                    "flex-1 py-2.5 text-sm font-bold rounded-lg shadow-sm bg-white text-slate-900 transition-all";
                btnStaff.className =
                    "flex-1 py-2.5 text-sm font-bold rounded-lg text-slate-500 hover:text-slate-700 transition-all";
                form.action = "client_portal.html";
            } else {
                btnStaff.className =
                    "flex-1 py-2.5 text-sm font-bold rounded-lg shadow-sm bg-white text-slate-900 transition-all";
                btnClient.className =
                    "flex-1 py-2.5 text-sm font-bold rounded-lg text-slate-500 hover:text-slate-700 transition-all";
                form.action = "index.html"; // Link to Admin Dashboard
            }
        }
    </script>
</body>
